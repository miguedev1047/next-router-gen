...
export function fillFileIfEmpty(filePath, template, fileType) {
    try {
        ...
        if (content.trim().length === 0) {
            ...
            console.log(`✅ Filled empty ${fileType}: ${path.relative(process.cwd(), filePath)}`);
        }
        else {
            console.log(`ℹ️ Skipped ${fileType} (already has content): ${path.relative(process.cwd(), filePath)}`);
        }
    }
    catch (error) {
